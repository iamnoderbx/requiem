import { Noise } from "shared/utilities/noise.utilities";

export abstract class Grasslands {
    public static getHeightAt(x: number, z: number): [number, number[]] {
        const baseNoise = Noise.Simplex(x, z, 0.0005, 2, 0.1, 2, 124);
        //return [baseNoise * 10, []];

        const exponent = 1.3;
        const exponentialNoise = math.pow(math.abs(1 + baseNoise), exponent);

        const mountainNoise = Noise.Simplex(x, z, 0.0001, 3, 0.025, 2, 12423);
        ...
        if (mountainNoise > 0.7) {
            ...
            const ridgedNoise = math.abs(Noise.Simplex(x, z, 0.0005, 3, 0.1, 6, 127));
        ...
        const riverNoise = Noise.Simplex(x, z, 0.00002, 2, 0.5, 4, 129);
