 ...
 * @author NodeSupport
 */
@Commands.Command({name : "addmarcs", description : "Add marcs to the given treasury identifier."})
export class ServerAddTreasuryMarcsCommand {
    constructor() {}
    /**
    ...
    @Commands.Arguments([
        ...
        {name : "amount", description : "The amount of marcs being added to the treasury.", type : NumberCommandType}
    ])
    ...
    command(interaction: CommandInteraction, id : number | undefined, amount : number | undefined) : CommandResponse {
        ...
        if(treasury) treasury.addMarcsToTeasuryByUser(amount, sender)
        else TreasuryAPIServiceRunner.addTreasuryMarcs(id, amount, sender.getUserId())

        return interaction.reply(`Successfully added ${amount} marcs to treasury ${id}.`)
    }
}
